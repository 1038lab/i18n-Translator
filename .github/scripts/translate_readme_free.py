...
class FreeTranslationRunner:
    ...
    def fix_navigation_table(self, text):
        ...
        # Add language entries dynamically based on enabled languages
        for lang_code in self.config.enabled_languages:
            if lang_code in self.config.languages_info:
                lang_info = self.config.languages_info[lang_code]
                file_suffix = lang_info['file_suffix']
                nav_lines.append(f"| {lang_info['name']} | [README{file_suffix}.md](./README{file_suffix}.md) | ✅ Available |")
...
<!-- AUTO-TRANSLATED -->"""
    ...
    def run(self):
        ...
        if not self.config.enabled and not force_translate:
            print("Translation disabled in config and not forced")
            return
...
<!-- ORIGINAL ENGLISH VERSION -->"""
        ...
        # Print summary
        print(f"Translation completed: {translated_count} files translated, {skipped_count} skipped, {error_count} errors")

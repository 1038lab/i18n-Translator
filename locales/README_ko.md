...
이 안내서를 사용하면 제공된 도구를 사용하여 영어 README.md 파일을 여러 언어로 자동 번역하기 위해 GitHub Actions 자동 변환 시스템을 신속하게 설정하는 데 도움이됩니다.
...
- ation ** 2 개의 번역 모드 ** : 무료 (Zero-Config) 및 API (Go품질)
...
4. 완료를 기다리Go 번역 된 파일의 `locales/` 폴더를 확인하십시오.

** 🎉 그게 다야! 없음 API 키가 필요합니다. **

---

## 💰 ** API 모드로 업그레이드 (선택 사항 - Go품질) **
...
2. 프로젝트 선택기를 클릭하Go 새 프로젝트를 만듭니다.
...
`.github/i18n-config.yml`을 열Go 모드를 변경하십시오.
...
- ** `translate-readme-api.yml` ** - API 모드 GitHub Actions 플로우 파일
- ** `translate-readme-free.yml` ** - 무료 모드 GitHub Actions 워크 플로 파일
...
`.github/i18n-config.yml` 파일을 열Go 필요에 따라 수정하십시오.
...
- ** `mode` ** : 번역 모드 -`"free"` (없음 API 키) 또는 `"api"` (Go품질)
...
```
** 참Go ** : `locales/` 폴더는 첫 번째 번역 중에 자동으로 작성됩니다.
...
- `README.md` 파일을 업데이트하Go `main` 지점으로 푸시 할 때
- 소스 파일이 번역 파일보다 새롭Go 재전송합니다 (증분 번역)

###  6.2 번역 결과 

각 번역 파일에는 다음이 포함됩니다.
- nav ** 아름다운 언어 탐색 테이블 ** - 쉬운 언어 스위칭을위한 Git발 아이콘 포함
- content ** 번역 된 컨텐츠 ** - 원래 형식 및 코드 블록 유지 관리
- foot ** 현지 바닥 글 정보 ** - 번역 노트 및 프로젝트 링크 포함
- ual️ ** 수동 편집 보호 ** - 사용자 수정을 자동으로 감지하Go 보호합니다.
...
```
- 0 GitHub Actions은 실행되지만 즉시 종료합니다
...
```
** 솔루션 ** : API 키가 올바르게 설정되어 있Go 충분한 권한이 있는지 확인하십시오.
...
- 모드의 경우 : "자동 번역 README (API)"워크 플로우를 사용하Go API 키가 설정되어 있는지 확인하십시오.

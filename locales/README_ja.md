...
このガイドは、提供されたツールを使用してGitHub Actions自動翻訳システムをすばやく設定して、英語README.mdファイルを複数の言語に自動的に翻訳します.
...
3. クリック**「ワークフローの実行」**→[`main`ブランチ]→[ワークフローの実行] **をクリックします**
...
###  3.2ファイルの説明
 ...
 - ** `translate-readme-free.yml` **  - フリーモードGitHub Actionsワークフローファイル
...
```
- ✅GitHub Actionsは実行されますが、すぐに終了します
...
### ヘルプを取得
 ...
 - [Issue](https://github.com/1038lab/i18n/issues)を送信します
 - アクション実行ログを確認します
